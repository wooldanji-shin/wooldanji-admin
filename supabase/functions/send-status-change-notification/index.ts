...
serve(async (req) => {
  try {
    ...
    console.log(`[FCM] 상태 변경 알림 요청: userId=${userId}, status=${status}`);

    // status 유효성 검사
    if (status !== "suspended" && status !== "inactive") {
      console.error(`[FCM] 잘못된 status 값: ${status}`);
      return new Response(
        JSON.stringify({ success: false, error: "status는 'suspended' 또는 'inactive'만 가능합니다" }),
    ...
    let rawTokens = user?.fcmToken;

    // user fcmToken 없으면 partner_users에서 fallback (파트너→아파트 전환 신청자)
    if (!rawTokens || rawTokens.length === 0) {
      console.log(`[FCM] user fcmToken 없음 — partner_users에서 조회: userId=${userId}`);
      ...
      rawTokens = partnerUser?.fcmToken;
    }
    ...
    if (status === "suspended") {
      title = "승인 보류";
      body = `${user.name}님, 승인이 보류되었습니다. 관리비,고지서 등 다시 첨부해주세요`;
      notificationType = "status_suspended";
    } else { // inactive
      title = "승인 취소";
      body = `${user.name}님, 30일동안 이용하지 않으셔서 승인이 취소되었습니다.`;
      notificationType = "status_inactive";
    }
