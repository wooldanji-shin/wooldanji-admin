 ...
 * - ad_subscriptions_v2.freeEndDate 기준으로 D-7인 running 광고 조회
...
serve(async (_req) => {
  try {
    ...
    console.log(`[ExpiryCheck] D-7 알림 대상 조회: ${targetStart.toISOString()} ~ ${targetEnd.toISOString()}`);
    ...
    if (!targets || targets.length === 0) {
      console.log(`[ExpiryCheck] D-7 알림 대상 없음`);
      return new Response(
    ...
    console.log(`[ExpiryCheck] D-7 알림 대상 ${targets.length}건 발견`);
    ...
    const { data: alreadySent } = await supabaseAdmin
      ...
      .select('"partnerUserId", "navigationParams"')
      .in('"partnerUserId"', partnerIds)
      .eq("type", "free_ad_expiry_warning")
      .gte('"createdAt"', todayStart.toISOString());

    const alreadySentKeys = new Set(
      (alreadySent ?? []).map((n: any) =>
        `${n.partnerUserId}_${n.navigationParams?.advertisementId}`
      )
    );
        ...
        const response = await fetch(
          `${SUPABASE_URL}/functions/v1/send-partner-fcm-notification`,
          {
            ...
            body: JSON.stringify({
              ...
              title: "무료광고 종료 안내",
              body: "무료광고 종료가 7일 남았습니다. 이후 결제될 예정입니다.",
