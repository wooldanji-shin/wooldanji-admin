...
serve(async (req) => {
  try {
    const { userId, inquiryId, replyContent } = await req.json();

    console.log(`[FCM] 문의 답변 알림 요청: userId=${userId}, inquiryId=${inquiryId}`);
    ...
    // 3. OAuth 2.0 액세스 토큰 생성
    const accessToken = await getAccessToken();
    console.log(`[FCM] 액세스 토큰 생성 완료`);

    // 4. 모든 FCM 토큰으로 푸시 알림 전송
          ...
          body: JSON.stringify({
            message: {
              ...
              android: {
                ...
                notification: {
                  sound: "default",
                  // clickAction 제거: 알림 탭 시 자동으로 앱의 메인 액티비티 실행
                },
              },
              apns: {
                payload: {
